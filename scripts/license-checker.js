...
const ignoreList = ["node-unrtf-win32-bin"];
...
async function checkLicenses() {
	...
	const copyLeftLicensesList = Object.keys(licenses).filter((license) => {
		...
		// Ignore dependencies in the ignoreList
		const packageName = license.split("@").slice(0, -1).join("@");
		if (ignoreList.includes(packageName)) {
			return false;
		}
